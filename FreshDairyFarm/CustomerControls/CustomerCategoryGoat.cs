    ...
    public partial class CustomerCategoryGoat : UserControl
    {
        ...
        private void cartBtn_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(textBoxId.Text))
            {
                ...
                {
                    string query = "INSERT INTO GoatCart (UserId, Id, Color, Weight, Age, Price) " +
                                   "VALUES (@UserId, @Id, @Color, @Weight, @Age, @Price)";
                    using (SqlCommand cmd = new SqlCommand(query, connection))
                    {
                        ...
                        string checkQuery = "SELECT COUNT(*) FROM GoatCart WHERE UserId = @UserId AND Id = @Id";
                        ...
                        else
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                MessageBox.Show("Goat added Successfully", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                BindGridView();
                                ResetControl();
                            }
                            else
